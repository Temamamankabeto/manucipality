...
export default function CreateCitizenPage() {
  ...
  const createCitizen = useCreateCitizenMutation((citizen) => {
    router.replace(`/dashboard/citizens/${citizen.id}`);
  });

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">Register Citizen</h1>
        <p className="text-muted-foreground">Create a draft registration and upload required documents before submission.</p>
      </div>
