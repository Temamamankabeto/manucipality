...
export default function SidebarContent({ collapsed = false }: SidebarContentProps) {
  ...
  function toggleMenu(label: string) { setOpenMenus((current) => ({ ...current, [label]: !current[label] })); }
      ...
      <nav className={cn("flex-1 space-y-5 overflow-y-auto", collapsed ? "p-2" : "p-4")}>
        {sections.map((section) => (
          <div key={section.title} className="space-y-2">
            {!collapsed && <p className="px-2 text-xs font-semibold uppercase tracking-wide text-sidebar-foreground/50">{section.title}</p>}
            <div className="space-y-1">
              {section.items.map((item) => {
                ...
                if (hasChildren) {
                  return <div key={item.label} className="space-y-1"><button type="button" title={collapsed ? item.label : undefined} onClick={() => toggleMenu(item.label)} className={cn("flex w-full items-center rounded-xl text-sm font-medium text-sidebar-foreground/80 transition hover:bg-sidebar-accent hover:text-sidebar-accent-foreground", collapsed ? "justify-center px-2 py-2" : "gap-2 px-3 py-2")}><Icon className="h-4 w-4 shrink-0" />{!collapsed && <><span className="flex-1 text-left">{item.label}</span><ChevronRight className={cn("h-3 w-3 shrink-0 transition-transform duration-200", isOpen && "rotate-90")} /></>}</button>{!collapsed && isOpen && <div className="ml-6 space-y-1 border-l border-sidebar-border pl-2">{item.children?.map((child) => <Link key={child.href} href={child.href} className={cn("flex items-center justify-between rounded-lg px-3 py-2 text-sm transition hover:bg-sidebar-accent hover:text-sidebar-accent-foreground", pathname === child.href && "bg-sidebar-primary text-sidebar-primary-foreground hover:bg-sidebar-primary hover:text-sidebar-primary-foreground")}><span className="truncate">{child.label}</span></Link>)}</div>}</div>;
                }
