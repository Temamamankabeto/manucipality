...
class HouseholdService
{
        ...
        if ($search !== '') {
            $query->where(function (Builder $q) use ($search) {
                $q->where('household_number', 'like', "%{$search}%")
                    ...
                    ->orWhereHas('headCitizen', function (Builder $citizen) use ($search) {
                        $citizen->where('first_name', 'like', "%{$search}%")
    ...
    public function create(array $data, User $actor): Household
    {
        return DB::transaction(function () use ($data, $actor) {
            $head = Citizen::query()
                ->with(['city', 'subcity', 'woreda', 'zone'])
                ->findOrFail($data['head_citizen_id']);

            $this->assertCitizenActive($head);

            $data = $this->locationFromCitizen($data, $head);
            ...
            $household = Household::create([
                ...
                'subcity_id' => $head->subcity_id,
                'woreda_id' => $head->woreda_id,
                'zone_id' => $head->zone_id,
                'house_number' => $data['house_number'] ?? $head->address?->house_number,
                'address' => $data['address'] ?? $head->address?->address,
            ...
            $this->activity('household.created', $household, $actor, [
                'head_citizen_id' => $head->id,
            ]);
            ...
            $this->notificationService->create(
                ...
                'household_created',
                'Household created',
                "Household {$household->household_number} was created."
            );
    ...
    public function update(Household $household, array $data, User $actor): Household
    {
        ...
        return DB::transaction(function () use ($household, $data, $actor) {
            ...
            $household->update([
                ...
                'woreda_id' => $head->woreda_id,
                'zone_id' => $head->zone_id,
                'house_number' => $data['house_number'] ?? $household->house_number,
    ...
    public function addMember(Household $household, array $data, User $actor): HouseholdMember
    {
            ...
            'is_dependent' => (bool) (
                ...
                ?? in_array(
                    ...
                    [
                        ...
                        HouseholdMember::RELATIONSHIP_DEPENDENT,
                    ],
                    true
                )
            ),
        ...
        $this->activity('household.member_added', $household, $actor, [
            'citizen_id' => $citizen->id,
        ...
        $this->notificationService->create(
            ...
            'Household membership added',
            "{$citizen->full_name} was added to household {$household->household_number}."
        );
    ...
    public function updateMember(
        ...
        $this->activity('household.member_updated', $household, $actor, [
            'member_id' => $member->id,
        ]);
    ...
    public function removeMember(Household $household, HouseholdMember $member, User $actor): void
    {
        ...
        if ($member->is_head) {
            throw ValidationException::withMessages([
                'member' => ['Household head cannot be removed. Assign another head first.'],
            ]);
        }

        $this->activity('household.member_removed', $household, $actor, [
            'member_id' => $member->id,
    ...
    public function transformPaginated(LengthAwarePaginator $households): array
    {
        return [
            ...
            'data' => collect($households->items())
                ->map(fn (Household $household) => $this->transform($household))
                ->values(),
            'meta' => [
    ...
    public function transform(Household $household): array
    {
        ...
        return [
            ...
            'members' => $household->members
                ->map(fn (HouseholdMember $member) => $this->memberPayload($member))
                ->values(),
    ...
    protected function locationFromCitizen(array $data, Citizen $citizen): array
    {
        $data['city_id'] = $citizen->city_id;
        $data['subcity_id'] = $citizen->subcity_id;
        $data['woreda_id'] = $citizen->woreda_id;
        $data['zone_id'] = $citizen->zone_id;
    ...
    protected function validateRelationship(
        ...
        if ($relationship === HouseholdMember::RELATIONSHIP_HEAD) {
            ...
            if ($existing) {
                throw ValidationException::withMessages([
                    'relationship' => ['A household can only have one active head.'],
                ]);
            }

            if ($this->citizenAge($citizen) < 18) {
                throw ValidationException::withMessages([
                    'citizen_id' => ['Household head must be at least 18 years old.'],
                ]);
            }
        }
        ...
        if ($relationship === HouseholdMember::RELATIONSHIP_SPOUSE && $isDependent) {
            throw ValidationException::withMessages([
                'is_dependent' => ['Spouse cannot be marked as dependent.'],
            ]);
        }
        ...
        if (in_array($relationship, [
            ...
            if ($this->citizenAge($citizen) >= 18 && $isDependent) {
                throw ValidationException::withMessages([
                    'is_dependent' => [
                        'Adult members cannot be marked as child/dependent without changing relationship to other.',
                    ],
                ]);
            }
        }
    ...
    protected function assertCitizenActive(Citizen $citizen): void
    {
        if (! in_array($citizen->status, [Citizen::STATUS_ACTIVE, Citizen::STATUS_APPROVED], true)) {
            throw ValidationException::withMessages([
                'citizen_id' => ['Only active citizens can be added to a household.'],
            ]);
        }
    }
    ...
    protected function assertCitizenNotActiveMember(int $citizenId, ?int $currentHouseholdId = null): void
    {
        $exists = HouseholdMember::query()
            ...
            ->when(
                $currentHouseholdId,
                fn ($query) => $query->where('household_id', '!=', $currentHouseholdId)
            )
            ->exists();

        if ($exists) {
            throw ValidationException::withMessages([
                'citizen_id' => ['Citizen already belongs to an active household.'],
            ]);
        }
    }
    ...
    protected function validateAddressMatchesCitizen(array $data, Citizen $citizen): void
    {
        foreach (['city_id', 'subcity_id', 'woreda_id', 'zone_id'] as $field) {
            if ((int) ($data[$field] ?? 0) !== (int) $citizen->{$field}) {
                throw ValidationException::withMessages([
                    $field => ['Household location must match the head citizen location.'],
                ]);
            }
        }
    }
    ...
    protected function assertCitizenBelongsToHouseholdScope(Household $household, Citizen $citizen): void
    {
        foreach (['city_id', 'subcity_id', 'woreda_id', 'zone_id'] as $field) {
            if ((int) $household->{$field} !== (int) $citizen->{$field}) {
                throw ValidationException::withMessages([
                    'citizen_id' => [
                        'Member must belong to the same city/subcity/woreda/zone as the household.',
                    ],
                ]);
            }
        }
    ...
    protected function assertMemberBelongsToHousehold(Household $household, HouseholdMember $member): void
    {
        if ((int) $member->household_id !== (int) $household->id) {
            throw ValidationException::withMessages([
                'member' => ['Member does not belong to this household.'],
            ]);
        }
    }
    ...
    protected function assertCanAccess(Household $household, User $actor): void
    {
        ...
        if (! $actor->isAdmin() || ! $actor->admin_level) {
            throw ValidationException::withMessages([
                'scope' => ['You are not allowed to access this household.'],
            ]);
        }
        ...
        if (! $column || ! $value || (int) $household->{$column} !== (int) $value) {
            throw ValidationException::withMessages([
                'scope' => ['Household is outside your administrative scope.'],
            ]);
        }
    }
    ...
    protected function assertOfficeScopeAllowed(array $data, User $actor): void
    {
        ...
        if (! $actor->isAdmin() || ! $actor->admin_level) {
            throw ValidationException::withMessages([
                'scope' => ['Selected household location is outside your administrative scope.'],
            ]);
        }
        ...
        if (! $column || ! $value || (int) ($data[$column] ?? 0) !== (int) $value) {
            throw ValidationException::withMessages([
                'scope' => ['Selected household location is outside your administrative scope.'],
            ]);
        }
    ...
    protected function nextHouseholdNumber(): string
    {
        ...
        $next = Household::query()
            ->where('household_number', 'like', $prefix . '-%')
            ->count() + 1;
